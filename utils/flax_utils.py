...
class EmpiricalMeanStdNormalizer(flax.struct.PyTreeNode):
    ...
    @classmethod
    def create(cls, shape, eps=1e-6, clip_max=10.0):
        return cls(
            eps=eps,
    ...
    def update_and_normalize(self, batch):
        normalizer = self.update(batch)
...
def save_normalizer(normalizer, save_dir, epoch):
    """Save the normalizer to a file.

    Args:
        normalizer: EmpiricalMeanStdNormalizer instance.
        save_dir: Directory to save the normalizer.
        epoch: Epoch number.
    """
    os.makedirs(save_dir, exist_ok=True)

    save_dict = dict(
        normalizer=flax.serialization.to_state_dict(normalizer),
    )
    save_path = os.path.join(save_dir, f"normalizer_{epoch}.pkl")
...
def restore_normalizer(normalizer, restore_path, restore_epoch):
    """Restore the normalizer from a file.

    Args:
        normalizer: EmpiricalMeanStdNormalizer instance used as a template.
        restore_path: Path pattern to the directory containing the saved normalizer.
        restore_epoch: Epoch number.
    """
    candidates = glob.glob(restore_path)
    assert len(candidates) == 1, f"Found {len(candidates)} candidates: {candidates}"

    load_path = os.path.join(candidates[0], f"normalizer_{restore_epoch}.pkl")

    with open(load_path, "rb") as f:
        load_dict = pickle.load(f)

    normalizer = flax.serialization.from_state_dict(
        normalizer,
        load_dict["normalizer"],
    )

    print(f"Restored normalizer from {load_path}")
