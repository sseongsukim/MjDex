...
# Adapted from https://github.com/clvrai/furniture-bench/blob/main/furniture_bench/utils/transform.py
...
def batch_euler2quat(euler):
    """
    Convert XYZ‐order Euler angles to quaternions (x, y, z, w), fully vectorized.
    ...
    if e.shape[-1] != 3:
        raise ValueError(f"Invalid shape for Euler angles: {e.shape}. Must end in 3.")

    # 반각(half-angle)
    half = 0.5 * e
    roll, pitch, yaw = half[..., 0], half[..., 1], half[..., 2]
    ...
    # 수치 오차 방지를 위해 정규화 (사실 이론상 이미 단위이지만, 배치 합성 등 후처리 안전성 확보)
...
def batch_mat2quat(rmat):
    """
    Converts given rotation matrix (or batch of rotation matrices) to quaternion(s).

    Args:
        rmat: Single 3x3 rotation matrix or batch of (batch_size, 3, 3) rotation matrices.
    Returns:
        vec4 float quaternion angles for single input, or (batch_size, 4) for batch input.
    """

    def single_mat2quat(M):
        m00 = M[0, 0]
        m01 = M[0, 1]
        m02 = M[0, 2]
        m10 = M[1, 0]
        ...
        # symmetric matrix K
        K = np.zeros((4, 4), dtype=np.float32)  # 명시적으로 float32로 초기화
        K[0, 0] = m00 - m11 - m22
        K[1, 1] = m11 - m00 - m22
        K[2, 2] = m22 - m00 - m11
        K[3, 3] = m00 + m11 + m22
    ...
    # Check if input is batch or single matrix
    if rmat.ndim == 2:  # Single 3x3 matrix
        return single_mat2quat(rmat)
    elif rmat.ndim == 3:  # Batch of (batch_size, 3, 3)
        batch_size = rmat.shape[0]
        quaternions = np.zeros((batch_size, 4), dtype=np.float32)
        for i in range(batch_size):
            quaternions[i] = single_mat2quat(rmat[i])
        return quaternions
    else:
        raise ValueError(
            "rmat must be a single (3, 3) matrix or a batch of (batch_size, 3, 3) matrices."
        )


def to_batch_homogeneous(
    pos: Union[list, npt.NDArray[np.float32]], rot: Union[list, npt.NDArray[np.float32]]
) -> npt.NDArray[np.float32]:
    """
    Given position (pos) and rotation (rot), convert them into a homogeneous transform matrix.
    ...
    Returns:
        - 단일 입력일 경우: (4, 4)
        - 배치 입력일 경우: (batch_size, 4, 4) 크기의 배열
    """
    # list 타입으로 들어올 수도 있으니 np.array로 변환
    if isinstance(pos, list):
        pos = np.array(pos, dtype=np.float32)
    if isinstance(rot, list):
        rot = np.array(rot, dtype=np.float32)

    # pos와 rot가 배치 형태인지 아닌지 ndim을 보고 구분
    if pos.ndim == 1:
        # pos: (3,)
        # rot: (3,3)
        assert rot.shape == (3, 3), "rot가 (3,3) 형태여야 합니다."
        ...
        # pos: (batch_size, 3)
        # rot: (batch_size, 3, 3)
        assert rot.ndim == 3, "배치 형태라면 rot는 (batch_size, 3, 3) 형태여야 합니다."
        batch_size = pos.shape[0]
        assert rot.shape[0] == batch_size, "pos와 rot의 batch_size가 일치해야 합니다."

        # 배치별로 homogeneous matrix 생성
        transforms = np.zeros((batch_size, 4, 4), dtype=np.float32)
        transforms[:, :3, :3] = rot
        transforms[:, :3, 3] = pos
        transforms[:, 3, 3] = 1.0
        return transforms
    else:
        raise ValueError("pos는 (3,) 또는 (batch_size, 3) 형태여야 합니다.")
